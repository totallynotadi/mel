...
def ffplay(song):
	...
	track = melo.ytmusic.search(song).tracks[0]
	opts = { 'loglevel': 'panic' }
	utils.get_music(track, track.name)
...
def queue_check():
	...
	while True:
		for song in utils.now_playing:
			...
			print(f'\r--- playing {track.name} - {artists}\n>>> ', end = '')
			utils.get_recs(track)
			utils.add_req()
			ffplay(song)

			print(f'\r--- done playing {track.name}\n>>> ', end = '')
			os.remove(os.path.join(os.path.expanduser('~'), 'Music', track.name + '.mp3'))
